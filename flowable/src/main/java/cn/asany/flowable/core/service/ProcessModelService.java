...
public class ProcessModelService {
    ...
    public ProcessModelService(ProcessModelDao processModelDao, ModelService modelService, FlowableModelQueryService flowableModelQueryService) {
    ...
    public ProcessModel importProcessModel(LoginUser loginUser, HttpServletRequest request, Part file) throws IOException {
        MultipartFile multipartFile = UploadUtils.partToMultipartFile(file);
        RemoteUser user = new RemoteUser();
        user.setId(loginUser.getUid().toString());
        SecurityUtils.assumeUser(user);
        ModelRepresentation modelRepresentation = this.flowableModelQueryService.importProcessModel(request, multipartFile);
