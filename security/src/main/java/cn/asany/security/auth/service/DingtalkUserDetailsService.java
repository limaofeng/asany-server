...
@Component
public class DingtalkUserDetailsService implements SimpleUserDetailsService<DingtalkAuthenticationToken.DingtalkCredentials> {
...
//    private UserService userService;
    ...
    @Override
    public UserDetails loadUserByToken(DingtalkAuthenticationToken.DingtalkCredentials credentials) throws UsernameNotFoundException {
...
//        }
//
//        Optional<Employee> optional = this.employeeService.findOneByLink(LinkType.dingtalk, user.getId());
//        if (optional.isPresent()) {
//            return userService.buildLoginUser(optional.get());
//        }
//        optional = this.employeeService.findOneByMobile(user.getMobile());
//        if (!optional.isPresent()) {
//            throw new DingtalkUserNotFoundException("未发现钉钉对应的用户，需要引导用户使用密码完成绑定操作", ObjectUtil.toMap(user));
//        }
//        if (!ObjectUtil.defaultValue(connected, Boolean.FALSE)) {
//            throw new DingtalkUserNotConnectedException("已发现钉钉对应的用户，需要引导用户完成绑定操作", ObjectUtil.toMap(user));
//        }
//        Employee employee = this.employeeService.connected(optional.get(), SocialProvider.dingtalk, user.getId());
