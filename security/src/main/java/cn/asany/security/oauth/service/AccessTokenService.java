...
public class AccessTokenService {
  ...
  // new AuthUser(appId, apiKey.getApplication().getName(), apiKey.getKey(),
  // apiKey.getDescription(), apiKey.getPlatform());

  //        String token = UUID.randomUUID().toString() + "_" + request.getGrantType() + "_" +
  // apiKey.getKey();
  //
  ...
  // hashOper.get(SecurityStorage.REFRESH_TOKEN_PREFIX + accessToken.getRefreshToken(), "token");
  ...
  //                }
  //                authService.retrieveUser(userDetails, (AuthUser)
  // hashOper.get(SecurityStorage.REFRESH_TOKEN_PREFIX + accessToken.getRefreshToken(), "user"));
  //                redisTemplate.delete(SecurityStorage.REFRESH_TOKEN_PREFIX +
  // accessToken.getRefreshToken());
  //                break;
  //        }

  //        accessToken.setKey(appId + "/" + DigestUtils.md5DigestAsHex(token.getBytes()));
  //        accessToken.setRefreshToken(appId + "/" + DigestUtils.md5DigestAsHex((token +
  ...
  //        redisTemplate.expire(redisRefreshTokenKey, accessToken.getReExpires(),
  // TimeUnit.SECONDS);
