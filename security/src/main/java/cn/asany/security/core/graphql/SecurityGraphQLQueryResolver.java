...
//    private GetObjectByID byID;
    ...
    public List<SecurityScopeObject> securityScopes(String organizationId, ContextInput contextInput, List<String> ids) {
//        if (contextInput == null) {
//            return this.securityService.getSecurityScopeObjects(organizationId, false, ids.stream().map(id -> SecurityScope.newInstance(id)).collect(Collectors.toList()), null,null);
//        } else {
//            return this.securityService.getSecurityScopeObjects(organizationId, contextInput.getParsing(), ids.stream().map(id -> SecurityScope.newInstance(id)).collect(Collectors.toList()), contextInput.getViewer(),null);
//        }
     ...
     * @return
     */
//    public List<ScopeData> queryScopes(List<String> params, ScopeContext context) throws NoSuchMethodException {
//        // 将字符串转换成表达式
...
//            boolean right = allCombinations.isRight(expressionObjectUp != null ? expressionObjectUp.getType() : null, s, expressionObject.getType());
//            // 函数中文名
...
//                String objectByID = byID.getObjectByID(expressionObject.getType(), expressionObject.getId());
//                attributes.add(ExpressionAttribute.builder().key("id").opts("=").value(expressionObject.getId()).title("主键").name(objectByID).build());
//            }
//            if (expressionObject.getAttributes() != null && expressionObject.getAttributes().size() > 0) {
//                for (ExpressionAttribute atr: expressionObject.getAttributes()) {
//                    SecurityExpressType type = expressionObject.getType();
//                    Map<String, String> queryTypeToAttributeMap = typeAttribute.getQueryTypeToAttributeMap(type);
//                    // 通过key  获取value
...
//                if (expressionObject.getDisplace() != null && expressionObject.getDisplace().getName() != null) {
//                    expressionDisplace = ExpressionDisplace.builder().name(expressionObject.getDisplace().getName()).index(expressionObject.getDisplace().getIndex()).title(FilterEnum.getFilterEnumValue(expressionObject.getDisplace().getName())).build();
//                }
//
...
//    }
    ...
    public List<RoleScope> roleScopes(BusinessFilter filter) {
        PropertyFilterBuilder builder = ObjectUtil.defaultValue(filter, new BusinessFilter()).getBuilder();
     ...
     * 查询角色分类
     */
    public RoleTypeConnection roleTypes(RoleTypeFilter filter, int page, int pageSize, OrderBy orderBy) {
        Pager<RoleType> pager = new Pager<>(page, pageSize, orderBy);
        PropertyFilterBuilder builder = ObjectUtil.defaultValue(filter, new RoleTypeFilter()).getBuilder();
        return Kit.connection(roleService.findTypePager(pager, builder.build()), RoleTypeConnection.class);
    }
     ...
     * 查询权限分类
     */
    public PermissionTypeConnection permissionTypes(PermissionTypeFilter filter, int page, int pageSize, OrderBy orderBy) {
        Pager<PermissionType> pager = new Pager<>(page, pageSize, orderBy);
        PropertyFilterBuilder builder = ObjectUtil.defaultValue(filter, new PermissionTypeFilter()).getBuilder();
        return Kit.connection(permissionService.findTypePager(pager, builder.build()), PermissionTypeConnection.class);
    }

    /**
     * 查询权限分类
     */
    public PermissionConnection permissions(PermissionFilter filter, int page, int pageSize, OrderBy orderBy) {
        Pager<Permission> pager = new Pager<>(page, pageSize, orderBy);
        PropertyFilterBuilder builder = ObjectUtil.defaultValue(filter, new PermissionFilter()).getBuilder();
        return Kit.connection(permissionService.findPager(pager, builder.build()), PermissionConnection.class);
    }
...
//    }
    ...
    public List<ScopeTypeResult> getTypeAttribute(Map<String, String> map){
        ...
        for (Map.Entry<String, String> m : map.entrySet()) {
            list.add(ScopeTypeResult.builder().key(m.getKey()).title(m.getValue()).type(ResultTypeScope.attribute).build());
        }
        return list;
    }
