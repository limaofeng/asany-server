...
//        return securityService.getSecurityScopeObjects(null, false,permission.getGrants().stream().map(item -> SecurityScope.newInstance(item.getSecurityType(), item.getValue())).collect(Collectors.toList()), null);
        return null;
    }

    public List<GrantPermission> grantPermissions(Permission permission) {
        List<GrantPermission> list = grantPermissionService.getGrantPermissionsByPermissionId(permission.getId());
        return list;
    }
}
