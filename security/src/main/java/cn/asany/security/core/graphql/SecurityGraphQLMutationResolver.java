...
//        }
        ...
        if (input.getGrants() != null) {
            user.setGrants(GrantPermissionUtils.allocation(SecurityType.user, user.getId().toString(), getGrantPermissionByUser(input.getGrants())));
        }
        return user;
    }

    private List<GrantPermission> getGrantPermissionByUser(List<GrantPermissionByUserInput> grantInputs) {
        return grantInputs.stream().map(item -> GrantPermission.builder()
                .permission(Permission.builder().id(item.getPermission()).build())
                .resource(item.getResource()).build()
...
//        }
    ...
    public Permission createPermission(PermissionUpdateInput input) {
        ...
        permission.setPermissionType(PermissionType.builder()
                .id(getPermissionTypeInput(input.getPermissionTypeInput())).build());
