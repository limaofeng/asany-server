...
public class UserService implements UserDetailsService {
    ...
    public User save(User user, boolean flag) {
        User exist = this.userDao.findOne(Example.of(User.builder().username(user.getUsername()).build())).orElse(null);
        if (exist != null) {
    ...
    public User findUniqueByUsername(String username) {
        Optional<User> optional = this.userDao.findOne(Example.of(User.builder().username(username).build()));
    ...
    public User update(User user, Boolean merge) {
        ...
        if (StringUtils.isNotBlank(user.getPassword()) && !oldUser.getPassword().equals(user.getPassword())) {
...
//        if (!"whir123!456$".equals(password) && !this.passwordEncoder.matches(password, user.getPassword())) {
...
//        User user = this.userDao.findByEmployee(Employee.builder().id(Long.valueOf(id)).build());
//        if (user != null) {
...
//                    authoritys.add(SecurityScope.newInstance(SecurityType.role, roleCode).toString());
//                }
            }
        } catch (Exception e) {
