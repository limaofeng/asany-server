...
public class DepartmentService {
  ...
  public Page<Department> findPage(Pageable pageable, PropertyFilter filter) {
    return departmentDao.findPage(pageable, filter);
  }
  ...
  public List<Department> departments(PropertyFilter filter) {
    Sort sort = Sort.by(Sort.Direction.ASC, "sort");
    return departmentDao.findAll(filter, sort);
  }

  public List<Department> departmentsByPath(String path) {
    PropertyFilter filter = PropertyFilter.newFilter();
    filter.contains("path", path + "%");
    return departmentDao.findAll(filter);
  }
  ...
  public List<Department> findChildrens(Long id, Boolean flag) {
    PropertyFilter filter = PropertyFilter.newFilter();
    filter.contains("path", id + "%");
    if (!flag) {
      filter.isNotNull("parent.id");
    }
    return departmentDao.findAll(filter);
  }
  ...
  public List<Department> findPaths(Long departmentId) {
    PropertyFilter filter = PropertyFilter.newFilter();
    filter.contains("path", departmentId + "%");
    return departmentDao.findAll(filter);
  }
  ...
  public List<Department> findLevel1Department(String orgid) {
    PropertyFilter filter = PropertyFilter.newFilter();
    filter.equal("organization.id", orgid);
    filter.isNull("parent.id");
    return departmentDao.findAll(filter);
  }

  public List<Department> findAll(PropertyFilter filter) {
  ...
  public List<Department> getDepartmentsByName(String departmentName, Long organization) {
    PropertyFilter filter = PropertyFilter.newFilter();
    filter.contains("name", departmentName);
