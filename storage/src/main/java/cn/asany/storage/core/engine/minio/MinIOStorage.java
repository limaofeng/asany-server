...
public class MinIOStorage implements Storage {
    ...
    public MinIOStorage(String endpoint, String accessKeyId, String accessKeySecret, String bucketName, boolean useSSL) {
    ...
    public void writeFile(String remotePath, File file) throws IOException {
        this.client.uploadObject(UploadObjectArgs.builder().filename(file.getAbsolutePath()).object(path(remotePath)).bucket(this.bucketName).build());
    }
    ...
    private FileObjectMetadata getObjectMetadata(String absolutePath, Item item) {
        StatObjectResponse stat = client.statObject(StatObjectArgs.builder().bucket(this.bucketName).object(absolutePath).build());
    ...
    private Optional<FileObjectMetadata> getObjectMetadata(String absolutePath) {
        try {
            StatObjectResponse statObjectResponse = client.statObject(StatObjectArgs.builder().bucket(this.bucketName).object(absolutePath).build());
            return Optional.of(FileObjectMetadata.builder()
