...
public class FileUploadService implements UploadService {
    ...
    public FileUploadService(FileService fileService, StorageResolver storageResolver) {
        ...
        StoragePlugin fragment = SpringContextUtil.createBean(FragmentStoragePlugin.class, SpringContextUtil.AutoType.AUTOWIRE_BY_TYPE);
        StoragePlugin base = SpringContextUtil.createBean(BaseStoragePlugin.class, SpringContextUtil.AutoType.AUTOWIRE_BY_TYPE);
    ...
    public FileObject upload(FileObject file, UploadOptions options) throws IOException {
        ...
        try {
            ...
            // 插件
            Set<String> plugins = new HashSet<>(ObjectUtil.defaultValue(space.getPlugins(), HashSet::new));
            // 添加默认插件
            plugins.add("base");
