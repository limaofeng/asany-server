...
public class BandwidthLimiter {
  ...
  /**
   * Set the max upload or download rate in KB/s. maxRate must be grater than 0. If maxRate is zero,
   * it means there is no bandwidth limit.
   *
   * @param maxRate If maxRate is zero, it means there is no bandwidth limit.
   * @throws IllegalArgumentException 检查异常
   */
  public synchronized void setMaxRate(int maxRate) throws IllegalArgumentException {
    ...
    if (maxRate == 0) this.timeCostPerChunk = 0;
    else this.timeCostPerChunk = (1000000000L * CHUNK_LENGTH) / (this.maxRate * KB);
  }
