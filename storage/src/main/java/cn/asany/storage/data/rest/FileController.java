...
//                return fileUploadService.upload(response.getBody(), contentType, StringUtil.defaultValue(info.getName(), ""), length, info.getSpace());
...
//    }
    ...
    public FileDetail view(@RequestParam("path") String path) {
        return fileService.findByPath(path.contains(":") ? path.substring(path.indexOf(":") + 1) : path);
    }
