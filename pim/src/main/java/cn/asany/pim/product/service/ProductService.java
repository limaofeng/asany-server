...
public class ProductService {
  ...
  public ProductService(ProductDao productDao, ProductArticleDao productArticleDao, ProductLinkageTypeDao productLinkageTypeDao) {
  ...
  public Optional<Product> addArticlesToProduct(Long productId, String linkType, List<Long> articleIds) {
    return this.productDao.findById(productId).map(product -> {
      ProductLinkageType linkageType = this.productLinkageTypeDao.findOneBy("code", linkType).orElseThrow(() -> new IllegalArgumentException("关联类型不存在"));
      for (Long articleId : articleIds) {
        if (this.productArticleDao.exists(PropertyFilter.newFilter()
          .equal("product.id", productId)
          .equal("article.id", articleId)
          .equal("linkageType.id", linkageType.getId()))) {
          continue;
        }
        this.productArticleDao.save(ProductArticle.builder()
  ...
  public Optional<Product> removeArticlesFromProduct(Long productId, String linkType, List<Long> articleIds) {
    return this.productDao.findById(productId).map(product -> {
      ProductLinkageType linkageType = this.productLinkageTypeDao.findOneBy("code", linkType).orElseThrow(() -> new IllegalArgumentException("关联类型不存在"));
      for (Long articleId : articleIds) {
        this.productArticleDao.findOne(PropertyFilter.newFilter()
          .equal("product.id", productId)
          .equal("article.id", articleId)
          .equal("linkageType.id", linkageType.getId())).ifPresent(this.productArticleDao::delete);
      }
      return product;
    });
  }
