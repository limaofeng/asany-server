...
public class ModelGroupService {
    ...
    public void autoGroup() {
        ...
        for (Map.Entry<String, ModelFieldState> entry : state.entrySet()) {
            clusterAnalyzer.addDocument(entry.getKey(), entry.getValue().current.getName() + ", " + entry.getValue().current.getCode());
        }
        ...
        for (Set set : list) {
            ...
            set.stream().forEach(item -> {
                ...
                state.get(item).words.wordList.stream().forEach(word -> {
                    if (word.getValue().length() <= 2) {
                        return;
                    }
                    if (ObjectUtil.exists(new String[]{"Query", "yml","创建", "查询", "更新", "新增", "修改", "删除"}, word.getValue())) {
                        return;
                    }
            ...
            Optional<Map.Entry<String, Integer>> optional = names.entrySet().stream().max((a, b) -> a.getValue() > b.getValue() ? 1 : -1);

            if (!optional.isPresent()) {
                continue;
            }
            System.out.println(optional.get().getKey() + "(" + optional.get().getValue() + ")" + "\t" + set);
        }
