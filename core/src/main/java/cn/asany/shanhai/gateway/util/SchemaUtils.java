...
public class SchemaUtils {
    ...
    public static List<ServiceSchemaVersionPatch> diff(GraphQLSchema prevSchema, GraphQLSchema graphQLSchema) {
        ...
        if (typeMap != null) {
            ...
            for (DiffObject typeDiffObject : allType) {
                if (typeDiffObject.getStatus() == DiffObject.DiffStatus.D) {
                    ...
                    items.add(ServiceSchemaVersionPatch.builder()
                        .name(type.getId())
                ...
                ServiceSchemaVersionPatch.ServiceSchemaVersionPatchBuilder builder = ServiceSchemaVersionPatch.builder();
                builder.name(type.getId()).type(type.getType()).description(type.getDescription()).status(typeDiffObject.getStatus());
                ...
                if (typeDiffObject.getStatus() == DiffObject.DiffStatus.A) {
                    ...
                    DiffObject fieldMap = ObjectUtil.find(typeDiffObject.getDiffObjects(), "path", typeDiffObject.getPath() + "/fieldMap");
                    if (fieldMap == null) {
                        continue;
                    }
                    for (DiffObject fieldDiffObject : fieldMap.getDiffObjects()) {
                        GraphQLField fieldDefinition = fieldDiffObject.getStatus() == DiffObject.DiffStatus.D ? ((GraphQLField) fieldDiffObject.getPrev()) : ((GraphQLField) fieldDiffObject.getCurrent());
...
//                }
    ...
    public static List<DiffObject> parse(List<DiffObject> diffs, String[] names) {
        ...
        if ("Mutation".equals(type)) {
            ...
            List<DiffObject> fieldMap = ObjectUtil.find(children, "path", "/mutationType/fieldMap").getDiffObjects();
            DiffObject field = ObjectUtil.find(fieldMap, "path", "/mutationType/fieldMap[" + fieldName + "]");
            return children;
        } else if ("Query".equals(type)) {
            return ObjectUtil.find(diffs, "path", "/queryType").getDiffObjects();
        } else {
            DiffObject typeMap = ObjectUtil.find(diffs, "path", "/typeMap");
            DiffObject typeDiffObject = ObjectUtil.find(typeMap.getDiffObjects(), "path", "/typeMap[" + type + "]");
