...
//    private RoleService roleService;
    ...
    public List<Department> departments(Organization organization, DepartmentFilter filter) {
        ...
        return departmentService.findAll(filter.getBuilder().equal("organization.id", organization.getId()).build());
    }

//    public EmployeeConnection employees(Organization organization, EmployeeFilter filter, int page, int pageSize, OrderBy orderBy) {
...
//        return Kit.connection(employeeService.findPager(pager, builder.build()), EmployeeConnection.class, new EmployeeEdgeConverter(item -> {
...
//        return roleService.getAllByOrg(organization.getId(), RoleScope.builder().id(scope).build());
//    }
