...
public class DepartmentService {
    ...
    public Department save(Department department) {
        ...
        department.setPath(department.getParent() == null ? department.getId() + PATH_SEPARATOR : department.getParent().getPath() + department.getId() + PATH_SEPARATOR);
        ...
        if (dep == null) {
            ...
            if (department.getParentId() != null) {
                ...
                if (parent != null) {
                    ...
                    Department save = this.importsave(Department.builder().sn(department.getParentId().toString()).organization(department.getOrganization()).build());
                }
            } else {
                department = this.departmentDao.save(department);
    ...
    public Position savePosition(Long departmentId, String positionName) {
        Job job = jobService.save(Job.builder().organization(this.departmentDao.getOne(departmentId).getOrganization()).name(positionName).build());
    ...
    public Department importupdate(Long id, boolean merge, Department department) {
        ...
        if (department.getParent() == null) {
            ...
            department.setParent(parent);
            department.setPath(parent == null ? department.getId() + PATH_SEPARATOR : parent.getPath() + department.getId() + PATH_SEPARATOR);
        }
        return this.departmentDao.update(department, merge);
    }
    ...
    public Department findOneByCode(String code) {
        Optional<Department> optional = this.departmentDao.findOne(Example.of(Department.builder().sn(code).build()));
        return optional.orElse(null);
    }

    public Department findOne(String name, Long parentId) {
        Optional<Department> optional = this.departmentDao.findOne(Example.of(Department.builder().name(name).parent(Department.builder().id(parentId).build()).build()));
        return optional.orElse(null);
    }
    ...
    public void addDepartmentLink(DepartmentLinkType type, Long id, String linkId) {
        Optional<DepartmentLink> optional = this.departmentLinkDao.findOne(Example.of(DepartmentLink.builder()
        ...
        if (optional.isPresent()) {
            ...
            DepartmentLink link = DepartmentLink.builder()
                .type(type)
                .department(Department.builder().id(id).build())
                .linkId(linkId)
                .build();
    ...
    public List<Employee> findChildrenDepartmentEmployee(Department department) {
        List<Department> all = departmentDao.findAll(Example.of(Department.builder().parent(department).build()));
        List<Employee> employeeList = new ArrayList<>();
        all.stream().forEach(item -> item.getEmployees().stream().forEach(it -> employeeList.add(it.getEmployee())));
        return employeeList;
    }
