...
public class OrganizationEmployeeStatusService {
    @Autowired
    private OrganizationEmployeeStatusDao employeeStatusDao;
    @Autowired
    OrganizationEmployeeDao organizationEmployeeDao;

    public OrganizationEmployeeStatus get(Long id){
        return this.employeeStatusDao.findOne(Example
                .of(OrganizationEmployeeStatus.builder().id(id).build())).orElse(null);
    }
    ...
    public OrganizationEmployeeStatus update(Long id,Boolean merge, OrganizationEmployeeStatus employeeStatus){
    ...
    public Boolean remove(Long id){
        List<OrganizationEmployee> organizationEmployeeDaoAll =
                organizationEmployeeDao.findAll(Example.of(OrganizationEmployee.builder()
                        .status(OrganizationEmployeeStatus.builder().id(id).build()).build()));
        // 组织人员状态已被员工使用则不可删除
