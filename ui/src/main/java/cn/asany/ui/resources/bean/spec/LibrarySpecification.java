...
public class LibrarySpecification implements Specification {
    ...
    static class HQLPredicate extends AbstractSimplePredicate {
        ...
        public String render(boolean isNegated, RenderingContext renderingContext) {
            String ids = StringUtil.join(this.libraries.stream().map(item -> item.toString()).toArray(String[]::new), ",");
            return "generatedAlias0.id in (SELECT resourceId FROM LibraryItem li WHERE li.library in (" + ids + "))";
        }
