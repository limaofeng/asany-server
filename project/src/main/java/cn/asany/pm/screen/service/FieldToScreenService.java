...
public class FieldToScreenService {
        ...
        fieldToScreenDao.findAll(
            Example.of(
                FieldToScreen.builder()
                    ...
                    .field(
                        FieldConfigurationItem.builder()
                            .field(Field.builder().id(field).build())
                            .build())
                    .build()));
    if (all.size() > 0) {
   ...
   * @date 2022/7/28 9:12
   */
  public List<Field> getIssueFields(Long id) {
    // 根据页面id，查询该页面拥有的字段
    List<FieldToScreen> all =
        fieldToScreenDao.findAll(
            Example.of(
    ...
    if (all.size() > 0) {
      for (FieldToScreen list : all) {
        // 根据字段id，查询该字段是否存在
        Field field = issueFieldDao.getOne(list.getField().getId());
        fields.add(field);
      }
      return fields;
    }
  ...
  public FieldToScreen setFieldToScreen(Long screen, Long tabPane, Long field) {
    ...
    if (field != null) {
      // 根据字段id，查询字段是否存在
      Field issueField = issueFieldDao.findById(field).orElse(null);
      if (issueField != null) {
        fieldToScreen.setField(FieldConfigurationItem.builder().field(issueField).build());
      }
    }
  ...
  public List<FieldConfigurationItem> getTabPaneFieldItem(Long id) {
    List<FieldToScreen> all =
        fieldToScreenDao.findAll(
            Example.of(
                FieldToScreen.builder()
                    ...
                    .build()));
    List<FieldConfigurationItem> stageFields = new ArrayList<>();
    if (all.size() > 0) {
      for (FieldToScreen list : all) {
        FieldConfigurationItem item = itemDao.findById(list.getField().getId()).orElse(null);
