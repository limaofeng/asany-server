 ...
 * **************************************************************
 */
package cn.asany.email.rrt.bean;
...
@NamedQueries({
  @NamedQuery(
      name = "selectUserDomainMapping",
      query =
          "SELECT rrt FROM JamesRecipientRewrite rrt WHERE rrt.user=:user AND rrt.domain=:domain"),
  @NamedQuery(name = "selectAllMappings", query = "SELECT rrt FROM JamesRecipientRewrite rrt"),
  @NamedQuery(
      name = "deleteMapping",
      query =
          "DELETE FROM JamesRecipientRewrite rrt WHERE rrt.user=:user AND rrt.domain=:domain AND rrt.targetAddress=:targetAddress"),
  @NamedQuery(
      name = "updateMapping",
      query =
          "UPDATE JamesRecipientRewrite rrt SET rrt.targetAddress=:targetAddress WHERE rrt.user=:user AND rrt.domain=:domain")
})
...
public class JPARecipientRewrite {
  ...
  /**
   * Default no-args constructor for JPA class enhancement. The constructor need to be public or
   * protected to be used by JPA. See: http://docs.oracle.com/javaee/6/tutorial/doc/bnbqa.html Do
   * not us this constructor, it is for JPA only.
   */
