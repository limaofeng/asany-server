...
import cn.asany.email.client.smtp.mail.*;
...
/** this interceptor is for client exchange information with server by smtp protocol */
public class ProtocolInterceptor implements Interceptor {
  ...
  protected void doMail() throws IOException {
    ...
    sink.writeUtf8("MAIL FROM: ").writeUtf8(from.canonicalAddress()).write(Utils.CRLF).flush();
  ...
  protected void doRcpt(Mailbox mailbox) throws IOException {
    sink.writeUtf8("RCPT TO: ").writeUtf8(mailbox.canonicalAddress()).write(Utils.CRLF).flush();
    String response = Response.readLine(source);
    if (Response.parseCode(response) != 250) throwException("RCPT", response);
  }

  private void doData() throws IOException {
    sink.writeUtf8("DATA").write(Utils.CRLF).flush();
    String response = Response.readLine(source);
  ...
  protected void doDataInternal() throws IOException {
    ...
    SmtpHeader.writeAllHeaders(sink, chain.mail().headers(), spec);
